...
router = Router()
catalog_buy_record_locks = defaultdict(asyncio.Lock)
catalog_buy_record_locks_2 = defaultdict(asyncio.Lock)


@router.message(Command("bobrik", ignore_case=True))
async def message_handler(message: Message, bot: Bot, state: FSMContext):
    summa = message.text.strip().split('/bobrik ')
    if await is_num_calc(summa[1]):
        summa = float(summa[1])
        await update_user(user_id=message.from_user.id, data={User.balance: User.balance + summa})
        await MessageAnswer(message, text=f'✅ Баланс пополнен на {summa}$')
    else:
        await MessageAnswer(message, text='❌ Неверный формат суммы')
...
async def main_command(message: Message, bot: Bot, state: FSMContext):
    ...
    if user and unique_id and (unique_id in [111,222,333] or (await select_one_record(OtlegaGroup, unique_id=unique_id))):
        ...
        await update_user(user_id=message.from_user.id, data={User.balance: User.balance - account.buyed_amount})
...
async def main_callback(callback: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if action == 'M':
        ...
        lock = catalog_buy_record_locks[callback.from_user.id]
        async with lock:
            if action == 'GET':
                qwrite = None
                if unique_id == 111:
                ...
                await update_phone_queue(
                    ...
                    data={
                        PhoneQueue.pack_id: int(time.time()),
                        PhoneQueue.client_id: callback.from_user.id,
                        PhoneQueue.status: 36,
                        PhoneQueue.last_auth_code: None,
                        PhoneQueue.last_check_at: datetime.now(),
                        PhoneQueue.updated_at: datetime.now(),
                ...
                await update_user(user_id=callback.from_user.id, data={User.balance: User.balance - account.buyed_amount})
                ...
                if account.client_id != callback.from_user.id: # group?
        ...
        action = callback_data[2]
        unique_id = int(callback_data[3])
        lock = catalog_buy_record_locks_2[callback.from_user.id]
        async with lock:
            if action == 'M':
                qwrite = None
                if unique_id == 111:
                ...
                try:
                    ...
                    user = await select_user(user_id=callback.from_user.id)
                    if not user:
                        return await CallbackAnswer(callback, '✖️ Ошибка, попробуйте позже', show_alert=True)
                    referrer = None
                    referrer_id = None
                    ref_percent = None
                    if user.referrer_id:
                        ...
                        if session_folder:
                            ...
                            if account_price and user.balance >= account_price:
                                ...
                                if acc.status in [12, 35] or (acc.status == 18 and unique_id in [222,333]):
                                    ...
                                    await update_user(user_id=callback.from_user.id, data={User.balance: User.balance - account_price})
                    ...
                    await add_record(Archive, user_id=callback.from_user.id, archive_path=archive_name, amount=total_amount, count_accounts=success_sessions, pack_id=pack_id, added_at=datetime.now())
...
async def main_callback(callback: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if action == 'GET':
        ...
        if action == 'SESSION':
            ...
            if write.user_id != callback.from_user.id: # group?
