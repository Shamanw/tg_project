...
async def main_callback(callback: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if action == 'M':
        ...
        if action == 'M':
            ...
            total_items = await select_many_records(PhoneQueue, count=True, drop_id=callback.from_user.id, status_in=[23,24], slet_at=period, slet_at_is_not_none=True)
            items_per_page = 12
            ...
            if writes:
                found_text = (
                    ...
                    f'\n<i>Номер • Сумма покупки • Время слёта</i>'
                )
                for write in writes:
                    text += (
                        f'\n<b><code>{write.phone_number}</code>{f" • {famount(write.payed_amount)}$" if write.payed_amount else ""} • {write.slet_at.strftime("%d.%m %H:%M")}</b>'
                    )
            else:
        ...
        if action == 'M':
            ...
            total_items = await select_many_records(PhoneQueue, count=True, drop_id=callback.from_user.id, status=18, slet_at=period, slet_at_is_not_none=True)
            items_per_page = 12
            offset = (page - 1) * items_per_page
            total_pages = (total_items + items_per_page - 1) // items_per_page
            writes = await select_many_records(
