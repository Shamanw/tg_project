...
async def main_callback(callback: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if action == 'M':
        total_count = 0
        total_amount = 0
        slet_count = await select_many_records(PhoneQueue, count=True, drop_id=callback.from_user.id, unban_month_status=0, status=18, slet_main_at='month', slet_main_at_is_not_none=True)
        slet_count += await select_many_records(PhoneQueue, count=True, drop_id=callback.from_user.id, unban_month_status=0, status=17, alive_status_in=[1,2,3], slet_main_at='month', slet_main_at_is_not_none=True)
        slet_count += await select_many_records(PhoneQueue, count=True, drop_id=callback.from_user.id, unban_month_status=0, status=17, alive_status_not_in=[1,2,3], alive_hold_status=1, buyed_at='month', buyed_at_is_not_none=True)
        nevalid_count = await select_many_records(PhoneQueue, count=True, drop_id=callback.from_user.id, unban_month_status=0, status_in=[23,24], slet_main_at='month', slet_main_at_is_not_none=True)
        if slet_count:
            total_count += slet_count
        if nevalid_count:
            total_count += nevalid_count
        if total_count:
            slet_amount = await select_phone_queues(drop_id=callback.from_user.id, unban_month_status=0, status=18, payed_amount_total=True, slet_main_at_month=True)
            slet_amount += await select_phone_queues(drop_id=callback.from_user.id, unban_month_status=0, alive_status_in=[1,2,3], status=17, payed_amount_total=True, slet_main_at_month=True)
            slet_amount += await select_phone_queues(drop_id=callback.from_user.id, unban_month_status=0, alive_status_not_in=[1,2,3], alive_hold_status=1, status=17, payed_amount_total=True, buyed_at_month=True)
            nevalid_amount = await select_phone_queues(drop_id=callback.from_user.id, unban_month_status=0, statuses=[23,24], payed_amount_total=True, slet_main_at_month=True)
            if slet_amount:
        ...
        if total_count and total_amount:
            ...
            if invoice_url and sql_id:
                writes = []
                slet_writes = []
                records = await select_many_records(PhoneQueue, drop_id=callback.from_user.id, unban_month_status=0, status=18, slet_main_at='month', slet_main_at_is_not_none=True)
                if records:
                    slet_writes += records
                records = await select_many_records(PhoneQueue, drop_id=callback.from_user.id, unban_month_status=0, status=17, alive_status_in=[1,2,3], slet_main_at='month', slet_main_at_is_not_none=True)
                if records:
                    slet_writes += records
                records = await select_many_records(PhoneQueue, drop_id=callback.from_user.id, unban_month_status=0, status=17, alive_status_not_in=[1,2,3], alive_hold_status=1, buyed_at='month', buyed_at_is_not_none=True)
                if records:
                    slet_writes += records
                nevalid_writes = await select_many_records(PhoneQueue, drop_id=callback.from_user.id, unban_month_status=0, status_in=[23,24], slet_main_at='month', slet_main_at_is_not_none=True)
                if slet_writes:
