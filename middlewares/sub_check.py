        ...
        if user:
            ...
            if bot:
                if bot.id == int(BOT_TOKEN.split(':')[0]):
                    ...
                    if bt.op_group_id and bt.op_group_link:
                        usr = await select_user(user_id=user.id)
                        if chat and chat.type == ChatType.PRIVATE and message and usr and usr.role == 'drop':
                            try:
                                member = await BotGetChatMember(bot, chat_id=bt.op_group_id, user_id=user.id)
                                # print(member)
                                if member and member.status not in ('member', 'administrator', 'creator'):
                                    return await MessageAnswer(
                                        message,
                                        text=f'<b>❗️ Вступите в <a href="{bt.op_group_link}">группу</a>, чтобы продолжить.</b>',
                                        reply_markup=subscribe_kb(bt.op_group_link),
